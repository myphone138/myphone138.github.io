...
# 脚本名称: 文件哈希计算与校验脚本
# 设计者: 王贵平
# 脚本存储路径: K:\完整性校验_6FEC A20A@A\hash tools
# 创建时间: $(Get-Date -Format "yyyy年M月d日 HH:mm:ss")
# 功能描述: 递归计算目录下所有文件的SHA256哈希值，生成CSV格式报告
# =============================================================================

$outputFile = "K:\完整性校验_6FEC A20A@A\王贵平本人对此仓库的签名文件\完整性校验_6FEC A20A@A完整哈希值.csv"
$counter = 1
...
$gpgPublicKeyPath = "完整性校验_6FEC A20A@A\王贵平本人对此仓库的签名文件\王贵平_0x6FECA20A_public.asc"
...
Get-ChildItem -Path "K:\完整性校验_6FEC A20A@A" -File -Recurse |
...
$csvContent += [PSCustomObject]@{
    序号 = "脚本存储路径"
    算法 = "完整性校验_6FEC A20A@A\王贵平本人对此仓库的签名文件\完整性校验_6FEC A20A@A批次哈希计算数据脚本代码.ps1"
    文件名 = ""
    哈希值 = ""
    大小 = ""
    创建时间 = ""
...
try {
    ...
    Write-Host "脚本设计者: 王贵平" -ForegroundColor Cyan
    Write-Host "脚本存储路径: "完整性校验_6FEC A20A@A\王贵平本人对此仓库的签名文件\完整性校验_6FEC A20A@A批次哈希计算数据脚本代码.ps1"" -ForegroundColor Cyan
